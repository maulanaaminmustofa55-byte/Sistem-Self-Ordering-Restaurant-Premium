...
// Validasi nomor meja (hanya 1, 2, 3)
$meja_valid = ['1', '2', '3'];
$no_meja = isset($_GET['meja']) && in_array($_GET['meja'], $meja_valid) ? $_GET['meja'] : '1';
...
if(isset($_POST['kirim'])){
    ...
    // Validasi meja hanya 1, 2, atau 3
    if (!in_array($meja, ['1', '2', '3'])) {
    ...
    $nama = trim($nama);
    if (empty($nama)) {
        echo "<script>alert('❌ Nama pelanggan wajib diisi!'); window.history.back();</script>";
        exit;
    }
    ...
    if($total_harga > 0){
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO pesanan (nama_pelanggan, nomor_meja, detail_pesanan) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $nama, $meja, $pesanan_final);
