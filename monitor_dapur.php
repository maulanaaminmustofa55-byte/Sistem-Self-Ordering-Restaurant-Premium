<?php
session_start();
// Simple auth: set password di config atau hardcode untuk demo
$KDS_PASSWORD = 'amin2024'; // Ganti di production!

if (!isset($_SESSION['kds_auth']) || $_SESSION['kds_auth'] !== true) {
    if (isset($_POST['kds_password'])) {
        if ($_POST['kds_password'] === $KDS_PASSWORD) {
            $_SESSION['kds_auth'] = true;
            header('Location: monitor_dapur.php');
            exit;
        } else {
            $auth_error = 'Password salah!';
        }
    }
    // Show login form
?>
...
<head>
    ...
    <title>KDS Login - Seafood Amin</title>
    ...
    <style>
        body { font-family: 'Poppins', sans-serif; background: #0a0a0a; min-height: 100vh; display: flex; align-items: center; justify-content: center; }
        .login-card { background: rgba(20,20,35,0.95); backdrop-filter: blur(10px); border: 1px solid rgba(212,175,55,0.2); border-radius: 24px; padding: 40px; width: 100%; max-width: 400px; }
        .login-title { font-family: 'Playfair Display', serif; color: #d4af37; font-size: 1.8rem; font-weight: 700; text-align: center; margin-bottom: 30px; }
        .form-control { background: rgba(255,255,255,0.08); border: 1px solid rgba(212,175,55,0.25); border-radius: 50px; padding: 14px 22px; color: white; }
        .form-control:focus { background: rgba(255,255,255,0.12); border-color: #d4af37; outline: none; box-shadow: 0 0 0 3px rgba(212,175,55,0.15); color: white; }
        .btn-login { background: linear-gradient(135deg, #d4af37, #b8960c); border: none; padding: 14px; border-radius: 50px; font-weight: 700; color: #0a0a0a; width: 100%; transition: all 0.3s; }
        .btn-login:hover { transform: scale(1.02); box-shadow: 0 10px 35px rgba(212,175,55,0.4); }
        .error-msg { color: #e74c3c; text-align: center; margin-bottom: 20px; }
    </style>
</head>
<body>
    <div class="login-card">
        <div class="login-title"><i class="fas fa-utensils"></i> KDS LOGIN</div>
        <?php if (isset($auth_error)): ?>
            <div class="error-msg"><?php echo $auth_error; ?></div>
        <?php endif; ?>
        <form method="POST">
            <div class="mb-3">
                <input type="password" name="kds_password" class="form-control" placeholder="Masukkan Password KDS" required autofocus>
            </div>
            <button type="submit" class="btn-login"><i class="fas fa-sign-in-alt"></i> MASUK KE DAPUR</button>
        </form>
    </div>
</body>
</html>
...
<div class="kds-header">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        ...
        <div class="d-flex align-items-center gap-3">
            <div class="live-clock" id="jam">00:00:00</div>
            <a href="?logout=1" class="btn-login" style="padding: 8px 20px; font-size: 0.85rem; text-decoration: none;">
